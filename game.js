...
`;
  ...
  function makeUI() {
    ...
    menu.innerHTML = `
      <div class="gPanel">
        ...
        <div style="display:flex;gap:10px;justify-content:flex-end">
          ...
          <button id="btnRanks" class="gBtn gBtnGhost">RANKINGS</button>
    ...
    ranks.className = "gOverlay";
    ranks.id = "ranks";
    ranks.style.display = "none";
    ranks.innerHTML = `
      <div class="gPanel">
        <h1 class="gTitle">RANKINGS</h1>
        <div class="gTiny gFade">Global if Leaderboard Worker is configured, otherwise local to this browser.</div>
        <div id="ranksBody" class="gDialog" style="max-height:40vh; overflow:auto; margin-top:10px"></div>
        <div style="display:flex;gap:10px;justify-content:flex-end;margin-top:12px">
          <button id="btnRanksRefresh" class="gBtn gBtnGhost">REFRESH</button>
          <button id="btnRanksClose" class="gBtn">CLOSE</button>
        </div>
      </div>
    `;
    ...
    return {
      menu, dialog, end, ranks,
      btnStart: menu.querySelector("#btnStart"),
      btnRanks: menu.querySelector("#btnRanks"),
      btnMute: menu.querySelector("#btnMute"),
      btnRestart: end.querySelector("#btnRestart"),
      ranksBody: ranks.querySelector("#ranksBody"),
      btnRanksClose: ranks.querySelector("#btnRanksClose"),
      btnRanksRefresh: ranks.querySelector("#btnRanksRefresh"),
  ...
  /********************************************************************
   * Leaderboard (local + optional Worker)
   ********************************************************************/
  const LB_KEY = "LFA_LB_V1";
  const Leaderboard = {
    endpoint: null, // set to your Worker URL if you have one
    async list(){
      // try remote first if configured
      if(this.endpoint){
        try{
          const r = await fetch(this.endpoint.replace(/\/$/,"") + "/top?limit=20", { cache:"no-store" });
          if(r.ok) return await r.json();
        }catch(_e){}
      }
      // local fallback
      try{ return JSON.parse(localStorage.getItem(LB_KEY) || "[]"); }catch(_e){ return []; }
    },
    async submit(row){
      // local
      let arr=[];
      try{ arr = JSON.parse(localStorage.getItem(LB_KEY) || "[]"); }catch(_e){ arr=[]; }
      arr.push(row);
      ...
      if(this.endpoint){
        try{
          await fetch(this.endpoint.replace(/\/$/,"") + "/submit", {
            method:"POST",
    ...
    async open(){
      UI.ranks.style.display="grid";
      await this.render();
    },
    close(){ UI.ranks.style.display="none"; },
    async render(){
      const rows = await this.list();
      const fmt = (n)=>String(n||0).padStart(6,"0");
      if(!rows || !rows.length){
        UI.ranksBody.textContent = "No records yet. Finish a run to etch your name.";
        return;
      }
      const lines = rows.slice(0,20).map((r,i)=>{
        const d = new Date(r.ts||Date.now());
        const ds = d.toISOString().slice(0,10);
        return `${String(i+1).padStart(2,"0")}  ${fmt(r.score)}  ${String(r.name||"UNKNOWN").padEnd(10," ")}  ${String(r.mission||"").padEnd(10," ")}  ${ds}`;
      }).join("\n");
      UI.ranksBody.textContent = "##  SCORE   NAME        MISSION     DATE\n" + lines;
    },
    onRunEnd(payload){ this.submit(payload); }
  };
  ...
  window.addEventListener("keydown", (e) => {
    ...
    // secret code capture (ignore when typing in the name field)
    ...
    if(e.key && e.key.length===1){
      ...
      if(ch>="A" && ch<="Z" || ch>="0" && ch<="9"){
        Input.codeBuf = (Input.codeBuf + ch).slice(-18);
        Input.codeTimer = 2.2;
      }
    }
  });
   ...
   ********************************************************************/
  class Assets {
    constructor(){ this.allowImages = true; this.img = new Map(); this.ready=false; this.manifest=[]; }
    ...
    async loadManifest(url="assets/manifest.json"){
      try{
        ...
        this.manifest = list.filter(Boolean);
  ...
  class VisualFX {
    ...
    triggerAssetGlitch(t=0.55){
      // pick any loaded image from manifest and flash it as hallucination
      const list = (ASSETS?.manifest||[]).slice();
      if(!list.length) return;
      const pick = list[randi(0,list.length)];
      const im = ASSETS.get(pick);
      if(im) this.triggerFace(im,t);
    }
    update(dt){
...
ctx.globalAlpha=1;
  ...
  class World {
    ...
    _genSecret(){
      // Pocket "secret room" far beyond the main ward.
      // Entered via spawned secret doors (WARD13 / encounters).
      const base = 10000;
      const gy = this.groundY;

      // structural walls (cosmetic collision)
      this.obstacles.push({x:base-40, y:gy-160, w:20, h:210});   // left wall
      this.obstacles.push({x:base+380, y:gy-160, w:20, h:210});  // right wall
      this.obstacles.push({x:base-40, y:gy-160, w:440, h:18});   // ceiling

      // exit marker inside the room
      this.secretExit = { x: base+340, y: gy-36, w: 22, h: 36 };

      // props inside: breakables + one chest
      const kinds = ["glass","bin","potion","glass","bin","glass","potion"];
      for(let i=0;i<kinds.length;i++){
        this.props.push(new Prop(base+40+i*46, gy-14, kinds[i]));
      }
      this.props.push(new Prop(base+240, gy-14, "chest"));
    }
    spawnSecretDoorNear(x){
      const sx = clamp(x, 120, this.exitX-260);
      this.secretDoors.push({ x:sx, y:this.groundY-34, w:20, h:34, open:true });
    }
    drawBackground(ctx,pal,t,mood){
      ctx.fillStyle=pal.bg; ctx.fillRect(0,0,RW,RH);

      // soft ambient lift (not pure black)
      const gAmb = ctx.createLinearGradient(0,0,0,RH);
      gAmb.addColorStop(0,"rgba(255,255,255,0.05)");
      gAmb.addColorStop(1,"rgba(0,0,0,0.00)");
      ctx.fillStyle=gAmb; ctx.fillRect(0,0,RW,RH);

      // window shafts
      ctx.globalAlpha=0.14;
      for(let i=0;i<4;i++){
        const wx = ((i*140 + (t*12)%140)|0) - 60;
        ctx.fillStyle="rgba(140,180,255,0.12)";
        ctx.beginPath();
        ctx.moveTo(wx,10); ctx.lineTo(wx+80,10);
        ctx.lineTo(wx+140,RH); ctx.lineTo(wx+60,RH);
        ctx.closePath(); ctx.fill();
      }
      ctx.globalAlpha=1;

      // ceiling lamps glow pools
      ctx.globalAlpha=0.22;
      for(let i=0;i<3;i++){
        const lx = 60 + i*170;
        ctx.fillStyle="rgba(255,240,200,0.14)";
        ctx.beginPath();
        ctx.ellipse(lx,40,42,18,0,0,Math.PI*2); ctx.fill();
        ctx.fillStyle="rgba(255,240,200,0.05)";
        ctx.beginPath();
        ctx.ellipse(lx,70,62,28,0,0,Math.PI*2); ctx.fill();
      }
      ctx.globalAlpha=1;
      ...
      const tags=["NO EXIT","DOCTOR LIES","WAKE UP","CAROSSA","DONT LOOK","I HEAR NEEDLES"];
      for(let i=0;i<10;i++){
        const gx = ((i*57 + (t*9)) % (RW+120)) - 60;
        const gy = 34 + (i%4)*22;
        const txtg = tags[(i+((t*0.2)|0))%tags.length];
        ctx.save();
        ctx.translate(gx,gy);
        ctx.rotate(((i%3)-1)*0.06);
        ctx.fillText(txtg,0,0);
        ctx.restore();
      }
      ctx.globalAlpha=1;
  ...
  class Player extends Entity {
    constructor(name, avi){
      ...
      this.status={ sleepT:0,  slowT:0, griefT:0, wiggleT:0, memoryT:0, oozeT:0 };
    ...
    applyEffect(kind,game){
      ...
      if(kind==="SLEEP"){
        // brief blackout / limp controls: you "fall asleep"
        this.status.sleepT = Math.max(this.status.sleepT, 5.5);
        this.status.slowT = Math.max(this.status.slowT, 5.5);
        game.fx.triggerVignettePulse(1.6);
        game.fx.triggerGlitch(0.8);
        game.whisper("YOUR EYELIDS ARE HEAVY. DO NOT TRUST THE LIGHT.", 2.0);
        AudioSys.ping("sleep", 0.9);
      }
      if(kind==="GLITCH"){
    ...
    update(dt,game){
      ...
      this.status.sleepT=Math.max(0,this.status.sleepT-dt);
      this.friendBoostT=Math.max(0,this.friendBoostT-dt);

      let dir = Input.readDir();
      // Sleep: limp, intermittent input loss
      if(this.status.sleepT>0){
        if(Math.random()<0.65) dir = 0;
  ...
  class Enemy extends Entity {
    constructor(x,y,type){
      super(x,y,18,26);
      this.type=type; // angel|fiend|golem|crazy|wailer|oozer|syringer
      this.touchDmg=5;
      ...
      if(type==="syringer"){ this.hp=22; this.speed=36; this.touchDmg=4; this.w=18; this.h=28; this.throwCD=rand(0.8,1.6); }
    }
    ...
    update(dt,game){
      ...
      if(this.type==="syringer"){
        // keeps mid-distance; throws sleep syringes
        const ad = Math.abs(dx);
        if(!game.madness.active){
          if(ad < 120) this.vx = -sign(dx)*this.speed*0.6;
          else if(ad < 260) this.vx = 0;
          else this.vx = sign(dx)*this.speed*0.9;

          this.throwCD -= dt;
          if(this.throwCD<=0 && ad < 320){
            this.throwCD = 1.35 + Math.random()*0.6;
            const ang = Math.atan2((p.cy-10)-this.cy, (p.cx)-this.cx);
            const sp = 220 + Math.random()*40;
            game.enemyProjectiles.push({ x:this.cx, y:this.cy-6, vx:Math.cos(ang)*sp, vy:Math.sin(ang)*sp-30, life:2.5, damage:6, kind:"SLEEP" });
            game.fx.triggerSpark(this.cx,this.cy-6, 0.6);
            if(chance(0.7)) AudioSys.ping("syr",0.8);
          }
        } else {
          this.vx = sign(dx)*this.speed*0.15;
        }
      }
  ...
  class Game {
    ...
    spawnParticle(x,y,kind){
      if(kind==="blood"){
        // richer blood spray + floor splats
        const n = 6 + randi(0,6);
        for(let i=0;i<n;i++) this.particles.push(new Particle(x+rand(-2,2), y+rand(-2,2), "blood", this.pal));
        if(Math.random()<0.8) this.particles.push(new Particle(x, y, "gib", this.pal));
        // decals roughly on ground line
        const gy = this.world.groundY + rand(-2,2);
        for(let i=0;i<3;i++) this.addDecal(x+rand(-14,14), gy, "blood");
      } else {
    ...
    update(dt){
      ...
      this.time += dt;
      if(Input.codeTimer>0){ Input.codeTimer-=dt; if(Input.codeTimer<=0) Input.codeBuf=""; }
      // secret codes
      if(Input.codeBuf.endsWith("CAROSSA")){
        Input.codeBuf=""; Input.codeTimer=0;
        this.fx.triggerAssetGlitch(0.8);
        this.fx.triggerGlitch(1.6);
        this.score.add(333, "SECRET: CAROSSA");
        this.badge.unlock("CAROSSA_WHISPER");
        this.whisper("THE NAME IS A KEY. THE KEY IS A WOUND.", 3.0);
      }
      if(Input.codeBuf.endsWith("WARD13")){
        Input.codeBuf=""; Input.codeTimer=0;
        this.score.add(130, "SECRET: WARD13");
        this.fx.triggerPaletteShift(4.0);
        this.whisper("WARD 13 OPENS INSIDE YOUR TEETH.", 3.0);
        this.world.spawnSecretDoorNear(this.player.x + 160);
      }
...
// secret room logic (enter via spawned doors, exit via marker)
if(!this.madness.active){
  if(this.secret.active){
    const base = 10000;
    // clamp within pocket bounds
    this.player.x = clamp(this.player.x, base-10, base+360);
    if(this.world.secretExit && Input.wantInteract()){
      const e = this.world.secretExit;
      if(aabb(this.player.x,this.player.y,this.player.w,this.player.h,e.x,e.y,e.w,e.h)){
        this.secret.active=false;
        this.player.x = this.secret.returnX;
        this.whisper("YOU EXIT THE POCKET OF WARD 13.", 2.4);
        this.fx.triggerGlitch(0.8);
        this.score.add(250, "EXIT SECRET");
      }
    }
  } else {
    // enter if interacting near a door
    if(Input.wantInteract()){
      for(const d of (this.world.secretDoors||[])){
        if(d.open && aabb(this.player.x,this.player.y,this.player.w,this.player.h,d.x,d.y,d.w,d.h)){
          d.open=false;
          this.secret.active=true;
          this.secret.returnX=this.player.x;
          this.player.x = 10040;
          this.player.y = this.world.groundY - this.player.h;
          this.fx.triggerGlitch(1.2);
          this.fx.triggerAssetGlitch(0.6);
          this.score.add(500, "ENTER SECRET");
          this.badge.unlock("WARD_13");
          this.whisper("WARD 13 IS A LIE INSIDE THE MAP.", 3.0);
          break;
        }
      }
    }
      ...
      if(this.madness.active && chance(0.018)) this.fx.triggerAssetGlitch(0.20);
      ...
      for(const pr of this.enemyProjectiles){
        ...
        if(!this.madness.active && pr.life>0){
          if(dist(pr.x,pr.y,p.cx,p.cy)<10){
            ...
            if(pr.kind==="SLEEP"){ p.applyEffect("SLEEP", this); }
            else if(!pr.noGrav && !this.madness.active){ p.applyEffect("CONFUSION", this); }
    ...
    draw(){
      ...
      // secret doors / secret exit marker
      for(const d of (this.world.secretDoors||[])){
        const x = (d.x - this.world.camx)|0, y = (d.y)|0;
        bctx.globalAlpha=0.9;
        bctx.fillStyle = "rgba(125,60,255,0.25)";
        bctx.fillRect(x,y,d.w,d.h);
        bctx.strokeStyle = "rgba(125,60,255,0.85)";
        bctx.strokeRect(x+0.5,y+0.5,d.w-1,d.h-1);
        bctx.globalAlpha=1;
        bctx.fillStyle="#e8e0ff";
        bctx.font="7px monospace";
        bctx.fillText("13", x+5, y+10);
      }
      if(this.secret.active && this.world.secretExit){
        const ex = (this.world.secretExit.x - this.world.camx)|0;
        const ey = this.world.secretExit.y|0;
        bctx.globalAlpha=0.8;
        bctx.fillStyle="rgba(255,255,255,0.08)";
        bctx.fillRect(ex,ey,this.world.secretExit.w,this.world.secretExit.h);
        bctx.strokeStyle="rgba(255,255,255,0.25)";
        bctx.strokeRect(ex+0.5,ey+0.5,this.world.secretExit.w-1,this.world.secretExit.h-1);
        bctx.globalAlpha=1;
      }
  ...
  // Rankings
  UI.btnRanks.onclick = async () => { await window.Leaderboard.open(); };
  UI.btnRanksClose.onclick = () => window.Leaderboard.close();
  UI.btnRanksRefresh.onclick = async () => { await window.Leaderboard.render(); };
  ...
  UI.btnStart.onclick = async () => {
    ...
    await (async()=>{
      const m = await ASSETS.loadManifest("assets/manifest.json");
      const merged = OPTIONAL_ASSET_LIST.concat(m||[]);
      const uniq = [...new Set(merged)];
      await ASSETS.loadList(uniq);
    })();
